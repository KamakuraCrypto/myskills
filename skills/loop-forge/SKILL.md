...
- Verification: **codex 5.5 + 5.4 (xhigh) + Opus 4.8 (and Fable 5 if available)**, fresh context, cross-model.
...
7. **Converge + escalate** — done only when every lens PASSES (or a finding is consciously accepted). Any
   money-path fork → build options + a recommendation, filter it once through Fable + codex, then
